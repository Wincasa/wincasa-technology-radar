import { select, arc, selectAll, event } from 'd3';
...
export function Radar(size, radar) {
    ...
    tip.direction(function () {
        if (select('.quadrant-table.selected').node()) {
            var selectedQuadrant = select('.quadrant-table.selected');
    ...
    function addRing(ring, order) {
        var table = select('.quadrant-table.' + order);
        table.append('h3').text(ring);
        return table.append('ul');
    }

    function calculateBlipCoordinates(blip, minRadius, maxRadius, startAngle) {
    ...
    function findBlipCoordinates(blip, minRadius, maxRadius, startAngle, allBlipCoordinatesInRing) {
        ...
        var coordinates = calculateBlipCoordinates(blip, minRadius, maxRadius, startAngle);
        var iterationCounter = 0;
        var foundAPlace = false;

        while (iterationCounter < maxIterations) {
            if (thereIsCollision(blip, coordinates, allBlipCoordinatesInRing)) {
                coordinates = calculateBlipCoordinates(blip, minRadius, maxRadius, startAngle);
            } else {
                foundAPlace = true;
                break;
            }
    ...
    function drawLegend(order) {
        ...
        var container = select('svg').append('g')
    ...
    function plotRadarHeader() {
        var header = select('body').insert('header', '#radar');
        header.append('div')
    ...
    function selectQuadrant(order, startAngle) {
        ...
        selectAll('.quadrant-group-' + order + ' .blip-link text').each(function () {
    ...
    self.init = function () {
        radarElement = select('body').append('div').attr('id', 'radar');
        return self;
    };
